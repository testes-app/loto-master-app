                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={styles.tabs}>
                    {CATEGORIES.map(cat => (
                        <TouchableOpacity
                            ...
                            style={[styles.tab, filtro === cat.id ? styles.tabActive : null]}
                        >
                            <Text style={[styles.tabText, filtro === cat.id ? styles.tabTextActive : null]}>{cat.label}</Text>
                        </TouchableOpacity>
                    ))}
                </ScrollView>
            </View>
                        ...
                        <View key={title} style={styles.secao}>
                            ...
                            <View style={styles.secaoBody}>
                                {items.map((item, idx) => (
                                    <View key={idx} style={[
                                        styles.card, 
                                        item.status === 'urgente' ? styles.cardUrgente : null, 
                                        item.status === 'atencao' ? styles.cardAtencao : null, 
                                        item.status === 'ok' ? styles.cardOk : null
                                    ]}>
                                        <Text style={styles.posicao}>{item.pos}</Text>
                                        <View style={styles.info}>
                                            <View style={styles.dezenas}>
                                                {item.dezenas.map((n, i) => (
                                                    <View key={i} style={[styles.bolinha, item.atraso >= 7 ? { backgroundColor: '#F43F5E' } : null]}>
